...
class BiffWorkbook
{
     ...
     * GetDate
     *
     * @param BiffWorkbookStyle $style
     * @param integer $value serial number of date
    ...
    private function _getDate (BiffWorkbookStyle $style, $value)
    {
        if ($value > 1)
        {
            // @todo changed $utcValue to simple mktime() because round() return a date with one day forward. What is the reason to use round() and $this->_datemode? -rh
            // $utcValue = round (($value - ($this->_datemode ? 24107 : 25569) + 1) * 86400);
            // possible: $utcValue = round ((($value-1) - ($this->_datemode ? 24107 : 25569) + 1) * 86400);
            // simple solution with mktime():
            $utcValue = mktime(0, 0, 0, 1, $value-1, 1900);
